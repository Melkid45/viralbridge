...
type ClaudeProviderBenchmarkRequest = {
  audit?: unknown;
  runs?: unknown;
};
...
export async function POST(request: NextRequest) {
  const modalUrl = process.env.MODAL_CLAUDE_PROVIDER_BENCHMARK_URL;
  const modalKey = process.env.MODAL_PROXY_TOKEN_ID;
  const modalSecret = process.env.MODAL_PROXY_TOKEN_SECRET;

  if (!modalUrl) {
    return configurationError("MODAL_CLAUDE_PROVIDER_BENCHMARK_URL");
  }
  ...
  let body: ClaudeProviderBenchmarkRequest;
  try {
    body = (await request.json()) as ClaudeProviderBenchmarkRequest;
  } catch {
    return NextResponse.json(
