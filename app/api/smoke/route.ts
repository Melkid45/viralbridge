...
export async function POST(request: NextRequest) {
  ...
  if (!modalUrl) return configurationError("MODAL_AGENT_URL");
  ...
  if (typeof body.url !== "string" || typeof body.prompt !== "string") {
    return NextResponse.json(
      {
        ok: false,
        stage: "next_validation",
        error: "Both url and prompt must be strings",
      },
      { status: 400 },
    );
  }

  if (body.prompt.length < 10 || body.prompt.length > 4_000) {
    return NextResponse.json(
      {
        ok: false,
        stage: "next_validation",
        error: "Prompt length must be between 10 and 4000 characters",
      },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    let modalPayload: unknown;

    try {
      modalPayload = JSON.parse(rawResponse);
    } catch {
      modalPayload = { raw_response: rawResponse.slice(0, 4_000) };
    }
